...
-(IBAction)trim: (id) sender
{
   ...
   float      xOrigin =
      ((imageWidth * scale - 320.0f) / 2.0f - self.draggableImageView.xMoved) / (scale);
   float      yOrigin =
      ((imageHeight * scale - 320.0f) / 2.0f - self.draggableImageView.yMoved) / (scale);
...
-(void)viewDidLoad
{
   ...
   self.draggableImageView.transform = CGAffineTransformScale(self.draggableImageView.transform,
                                                              ratio, ratio);
